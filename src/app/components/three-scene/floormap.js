...
export default class Floormap {
    constructor(parameters) {
        this.onLoad = function (description) {
            // Store the floormap's map and size
            ...
            // Build the floor map
            let wallObject;
            for (let i = 0; i <= description.size.width; i++) { // In order to represent the eastmost walls, the map width is one column greater than the actual floor map width
                for (let j = 0; j <= description.size.height; j++) { // In order to represent the southmost walls, the map height is one row greater than the actual floor map height
                    /*
        ...
        // Load a floor map description resource file
        loader.load(
            //Resource URL
            this.url,
